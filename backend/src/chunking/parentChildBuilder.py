...
from src.chunking.structureAwareChunker import calculate_normalized_hash
...
class ParentChildBuilder:
    ...
    def __init__(self, child_chunk_size: int = 400, child_chunk_overlap: int = 50) -> None:
    ...
    def _generate_children(
        ...
        if len(text) <= self.child_chunk_size:
            child = self._build_child(text, parent, parent_id, 0)
            return [child]
        ...
        for sentence in sentences:
            ...
            if not current_sentences:
                if sentence_length > self.child_chunk_size:
                    ...
                    for part_text in sentence_parts:
                        child_chunks.append(
                            self._build_child(
                                part_text, parent, parent_id, len(child_chunks)
                            )
                        )
                else:
            ...
            if current_size + 1 + sentence_length <= self.child_chunk_size:
                ...
                child_chunks.append(
                    self._build_child(
                        child_text, parent, parent_id, len(child_chunks)
                    )
                )
    ...
    def _build_child(
        self, text: str, parent: DocChunk, parent_id: UUID, chunk_index: int
    ) -> ChildChunk:
        ...
        return ChildChunk(
            ...
            ingestion_version=parent.ingestion_version,
